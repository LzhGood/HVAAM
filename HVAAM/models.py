...
def repeat(x):
    return K.reshape(K.batch_flatten(x), (b_s, 512, shape_r_gt, shape_c_gt))
    #return x

def repeat_shape(s):
    return (s[0], 512, shape_r_gt, shape_c_gt)
    #return s
...
def upsampling_LZH(x,y):
    return T.nnet.abstract_conv.bilinear_upsampling(input=x, ratio=2, num_input_channels=y, batch_size=b_s)

def upsampling_shape_LZH(s):
    return s[:2] + (s[2] * 2, s[3] * 2)
# KL-Divergence Loss
...
def hvaam(x):
    ...
    concateneted = merge([dcn.output, priors1], mode='concat', concat_axis=1)
    learned_priors1 = AtrousConvolution2D(512, 5, 5, border_mode='same', activation='relu',                                          atrous_rate=(4, 4), W_regularizer=l2(regular_size_))(concateneted)
    ...
    learned_priors2 = AtrousConvolution2D(512, 5, 5, border_mode='same', activation='relu',
                                          atrous_rate=(4, 4), W_regularizer=l2(regular_size_))(concateneted)

    # Final Convolutional Layer
    # outs = Convolution2D(1, 1, 1, border_mode='same', activation='relu')(learned_priors1)
    # outs_up = Lambda(upsampling, upsampling_shape)(outs)

    outs1 = Lambda(upsampling_LZH, upsampling_shape_LZH, arguments={'y': 512}, name="Up_lzh_1")(learned_priors2)
    outs2 = Convolution2D(128, 3, 3, border_mode='same', activation='relu', name="Conv_lzh_1",
                          W_regularizer=l2(regular_size_))(outs1)
    outs2 = Dropout(0.5)(outs2)
    outs3 = Lambda(upsampling_LZH, upsampling_shape_LZH, arguments={'y': 128}, name="Up_lzh_2")(
        outs2)  # 输出为saliency map
    outs4 = Convolution2D(64, 3, 3, border_mode='same', activation='relu', name="Conv_lzh_2",
                          W_regularizer=l2(regular_size_))(outs3)
    outs4 = Dropout(0.5)(outs4)
    outs5 = Lambda(upsampling_LZH, upsampling_shape_LZH, arguments={'y': 64}, name="Up_lzh_3")(
        outs4)  # 输出为saliency map
    outs6 = Convolution2D(32, 3, 3, border_mode='same', activation='relu', name="Conv_lzh_3",
                          W_regularizer=l2(regular_size_))(outs5)
    outs6 = Dropout(0.5)(outs6)
    outs7 = Convolution2D(1, 1, 1, border_mode='same', activation='relu', name="Conv_lzh_4",
                          W_regularizer=l2(regular_size_))(outs6)
    outs_up = Lambda(upsampling_LZH, upsampling_shape_LZH, arguments={'y': 1}, name="Up_lzh_4")(outs7)
