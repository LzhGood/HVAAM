...
def dcn_vgg(input_tensor=None):
    ...
    # conv_5 空洞卷积层 lzh5
    x = AtrousConvolution2D(512, 3, 3, activation='relu', border_mode='same', name='block5_conv1',
                            atrous_rate=(2, 2), W_regularizer=l2(regular_size_))(x)
    x = Dropout(0.5)(x)
    x = AtrousConvolution2D(512, 3, 3, activation='relu', border_mode='same', name='block5_conv2',
                            atrous_rate=(2, 2), W_regularizer=l2(regular_size_))(x)
    x = Dropout(0.5)(x)
    x = AtrousConvolution2D(512, 3, 3, activation='relu', border_mode='same', name='block5_conv3',
                            atrous_rate=(2, 2), W_regularizer=l2(regular_size_))(x)
    x = Dropout(0.5)(x)

    # Create model
    model = Model(img_input, x)  # (input,output)
    # Load weights
    # weights_path = get_file('vgg16_weights_th_dim_ordering_th_kernels_notop.h5', TH_WEIGHTS_PATH_NO_TOP,
    #                        cache_subdir='models')
    #use local model weight
