...
def preprocess_images(paths, shape_r, shape_c):
    ...
    for i, path in enumerate(paths):
        ...
        padded_image = padding(original_image, shape_r, shape_c, 3)#resize image
        ims[i] = padded_image
...
#         coords = []
#         for gaze in gazes:
#             coords.extend(gaze[0][2])#在列表末尾一次性追加另一个序列中的多个值（用新列表扩展原来的列表）。
#         for coord in coords:
...
def preprocess_fixmaps(paths, shape_r, shape_c):
    ...
    for i, path in enumerate(paths):
        fixmap=cv2.imread(path,0)
        ims[i,0]=fixmap.astype(np.float32)
